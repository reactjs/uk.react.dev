...
Існують два види побічних еффектів в React-компонентах: ті, які потребують і ті, які не потребують очистки. Давайте розглянемо обидва приклади в деталях.

## Еффекти без очистки {#effects-without-cleanup}

Інколи ми хочемо **запустити додатковий код після того, як React оновив DOM.** Мережеві запити, ручні DOM-мутації та логування є прикладами еффектів, які не потребують очистки. Це тому, що ми їх запускаємо і після цього відразу забуваємо про них, оскільки більше ніяких додаткових дій не потрібно. Давайте порівняємо, як класи та хуки дозволяють реалізовувати такі побічні еффекти.
