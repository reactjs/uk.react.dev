...
Раніше ви могли знати, що такі компоненти відомі як "компоненти без стану". Зараз ми покажемо, як додавати до них стан, а тому ми будемо називати їх "функціональні компоненти".

Хуки **не** процюють всередині класів. Але ви можете використовувати їх замість написання класів.
...
```js
  var fruitStateVariable = useState('banana'); // Повертає пару
...
>Примітка
>
>Ви можете поцікавитись, як React знає про те, якому компоненту відповідає `useState`, оскільки ми не передаємо йому нічого подібного до `this`. Ми дамо відповідь на [це](/docs/hooks-faq.html#how-does-react-associate-hook-calls-with-components) та багато інших питань у розділі FAQ.
...
Також ми дізнались більше про хуки в цілому. Хуки — це функції, що дозволяє вам "зачепитись" за можливості React у функціональних компонентах. Їхні імена завжди починаються з `use`. Також існує ще кілька хуків, про які ми поки що не знаємо.
