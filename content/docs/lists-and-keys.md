...
### Рендер декількох компонентів {#rendering-multiple-components}
...
Тепер ми включимо масив `listItems` цілком всередину елемента `<ul>`, і [рендерити його у DOM](/docs/rendering-elements.html#rendering-an-element-into-the-dom):
...
Цей код виведе маркер список чисел від 1 до 5.
...
Коли ви запустите цей код, ви отримаєте попередження, що для елементів списку має бути вказано ключ. "Ключ" - це спеціальний строковий атрибут, який потрібно вказувати при створенні списку елементів. Ми обговоримо, чому це важливо, у наступній секції.
