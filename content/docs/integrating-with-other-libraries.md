...
Одне з рішень — з кожною зміною моделі, «витягувати» атрибути моделі як прості дані і зберігати всю цю логіку в одному місці. Ось — [компонент вищого порядку](/docs/higher-order-components.html) отримує всі атрибути Backbone-моделі та передає їх до компоненту що огортається.
