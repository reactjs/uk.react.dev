...
Наша нова вимога полягає в тому, що, крім поля вводу за Цельсієм, ми надамо можливість вводити температуру за Фаренгейтом, і вони будуть синхронізовані між собою.

Ми можемо почати з того, що дістанемо компонент `TemperatureInput` з `Calculator`. До нього додамо проп `scale` (шкала), який може бути `"c"` або `"f"`:
...
Для початку ми напишемо дві функції для перетворення температури з градусів Цельсія на Фаренгейт і назад:
...
Ці дві функції перетворюють числа. Ми напишемо ще одну функцію, яка приймає два аргумента: рядок `temperature` і функцію перетворення `convert`, і повертає рядок. Ми будемо використовувати його для обчислення значення одного поля входу на основі іншого.
...
Спершу ми замінемо `this.state.temperature` на `this.props.temperature` в компоненті `TemperatureInput`. Наразі давайте представимо, що `this.props.temperature` вже існує, хоча в майбутньому нам доведеться передавати його з `Calculator`:
...
* У середині цих методів компонент `Calculator` просить React повторно зробити рендер самого себе, шляхом виклику `this.setState()` з новим введеним значенням і поточною шкалою вводу, яку ми щойно редагували.
