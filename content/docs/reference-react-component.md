...
Функція `render()` має бути чистою, а це означає, що вона не змінює стан компонента, повертає однаковий результат при кожному викликі і не взаємодіє з браузером напряму.
...
**Не варто викликати `setState()`** в `constructor()`. Натомість, якщо компонент потребує використання локального стану, **присвоюйте початкове значення `this.state`** безпосередньо в конструкторі:
...
Також цей метод — вдале місце для налаштування підписок. Якщо ви це зробите, то не забудьте відписатись в `componentWillUnmount()`.
...
Це виконає поверхове об'єднання `stateChange` в новий стан, наприклад, для зміни кількості товарів в корзині:
