...
Забезпечити таку поведінку було нескладно. Проте, задумайтесь, як це взагалі може працювати, і зможете трохи збитися з пантелику. Якщо ми змінюємо стан, то чому ж ми негайно не бачимо зміни? *Де* проходить наступний рендер компоненту `<ProfilePage>`?
...
У даному прикладі ми починаємо завантажувати дані разом з появою компоненту, *а також* щоразу при натисканні на кнопку "Refresh". Ми поміщаємо результат виклику `fetchUserAndPosts()` у стан, щоб нижчеописані компоненти могли почати читати нові дані з тільки що відправленого запиту.
...
Вже можна помітити наскільки *часто* `useTransition` може стати в нагоді. Практично будь-який клік по кнопці, чи взаємодія, що призводитиме до призупинки компоненту, повинні бути огорнуті в `useTransition`, щоб запобігти прихованню того, з чим користувач взаємодіє.
...
### Спосіб по замовчуванню: Відступ → Кістяк → Завершений {#default-receded-skeleton-complete}
...
У цьому прикладі, ми почали свій шлях з `<HomePage>`:
...
### Вдосконалений сценарій: Очікування → Кістяк → Завершений {#preferred-pending-skeleton-complete}
...
* Сценарій по замовчуванню: [Відступ → Кістяк → Завершений](https://codesandbox.io/s/prod-grass-g1lh5)
* **Вдосконалений сценарій: [Очікування → Кістяк → Завершений](https://codesandbox.io/s/focused-snow-xbkvl)**
...
* По замовчуванню, порядок завантаження виглядає наступним чином: Відступ → Кістяк → Завершений.
...
Щоб продемонструвати дану особливість, використаємо [цей приклад з перемикаємими профілями](https://codesandbox.io/s/musing-ramanujan-bgw2o). Натисніть на кнопку "Next" і зверніть увагу, що зміна профілю займає 1 секунду.
...
У цьому прикладі **кожен елемент у `<MySlowList>` штучно сповільнено — кожен з них блокує наступні на кілька мілісекунд**. Ми б ніколи не робили такого в реальному додатку, але цей приклад допоможе нам симулювати те, що відбувається в глубокому дереві компонентів, де немає явного простору для оптимізацій.
...
Паралельний режим пропонує потужну модель для розробки UI, разом з набором нових, компонуємих примітивів, щоб допомогти вам створювати інтерфейси, від взаємодії з якими ваші користувачі залишаться в захваті.
