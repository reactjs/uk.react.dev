...
> Примітка:
>
> Приклади нижче були оновлені для використання `React.createRef()` API, що з'явилося у React 16.3. Якщо ви користуєтеся попередніми версіями React, ми рекомендуємо використовувати [рефи зворотнього виклику](#callback-refs).
...
- **Заборонено використовувати атрибут `ref` до функціональних компонентів**, тому що у них немає екземплярів.
...
React зв'яже властивість `current` з DOM-елементом, коли компонент буде примонтований, та встановить назад в `null`, коли компонент буде прибрано з DOM. Оновлення `ref` відбувається перед `componentDidMount` або `componentDidUpdate`.
...
React викличе функцію зворотнього виклику `ref` з DOM-елементом, коли компонент буде примонтований, та виконає її зі значення `null`, коли компонент буде прибрано. Рефи гарантують актуальність перед викликом метода `componentDidMount` або `componentDidUpdate`.
...
### Застралий API: рядкові рефи {#legacy-api-string-refs}
