...
Знаходить усі елементи DOM-компонентів у рендер-дереві, що є DOM-компонентами з іменем класу, відповідним `className`.
...
Подібно [`scryRenderedDOMComponentsWithClass()`](#scryrendereddomcomponentswithclass), але очікує, що буде один результат, і поверне його або викине виключення, якщо кількість співпадінь більше одиниці.
...
Подібно [`scryRenderedDOMComponentsWithTag()`](#scryrendereddomcomponentswithtag), але очікує, що буде один результат, і поверне його або викине виключення, якщо кількість співпадінь більше одиниці.
...
Такий самий, як [`scryRenderedComponentsWithType()`](#scryrenderedcomponentswithtype), але очікує, що буде один результат, і поверне його або викине виключення, якщо кількість співпадінь більше одиниці.
...
Рендерить React-елемент в окремий вузлол DOM у документі. **Ця функція вимагає DOM.** Це фактично еквівалентно:
